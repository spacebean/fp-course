...
instance Applicative Gen where
  ...
  Gen g <*> Gen h = Gen $ \size gen ->
    let (genG, genH) = split gen
     in g size genG $ h size genH

instance Monad Gen where
  Gen g >>= f = Gen $ \size gen ->
    let (genG, genA) = split gen
     in runGen (f (g size genG)) size genA
...
instance Coarbitrary Integer where
  coarbitrary n = variant (fromInteger i)
    where
      i
        | n >= 0 = 2 * n
        | otherwise = 2 * (- n) + 1
