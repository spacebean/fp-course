...
import qualified Prelude as P (const, fmap, return, (>>=))
...
-- Full 8
--
-- prop> \a b c x y z -> (a :. b :. c :. Nil) *> (x :. y :. z :. Nil) ==
...
-- Full 7
--
-- prop> \x y z a b c -> (x :. y :. z :. Nil) <* (a :. b :. c :. Nil) ==
-- (x :. x :. x :. y :. y :. y :. z :. z :. z :. Nil)
--
...
filtering ::
  ...
  k (List a)
filtering f as =
  lift1 flatten (lift2 (zipWith (\b a -> if b then a :. Nil else Nil)) (sequence (map f as)) (pure as))
